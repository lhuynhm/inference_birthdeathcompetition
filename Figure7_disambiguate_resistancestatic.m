...
close all; clear all;
%% Case 1: static, gamma = 0
% Parameters
gamma_vec        = [0; 1; 0.5]; 
b0_vec_original  = [(1.1)/120; (1.1)/120; (1.1)/120];
d0_vec_original  = [0.1/120; 0.1/120; 0.1/120]; 
b0_vec           = b0_vec_original;
d0_vec           = d0_vec_original;
...
%% Case 2: static, gamma = 0.5
% Parameters
gamma_vec        = [0.5; 0.5; 0.75];  
b0_vec_original  = [(1.1)/120; (1.1)/120; (1.1)/120];
d0_vec_original  = [0.1/120; 0.1/120; 0.1/120]; 
r_vec_original   = b0_vec_original - d0_vec_original;
b0_vec           = b0_vec_original;
d0_vec(1)        = max(d0_vec_original(1) - 2*r_vec_original(1),0);
d0_vec(1)        = 0;
K_vec            = [(2/3)*1e3; 1e3; 0.5*1e3];
binsz_orig       = 10;
nrun             = 100;
for i = 2:3
    switch i 
